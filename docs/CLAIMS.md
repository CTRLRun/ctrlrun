...
| "records what actually happened" | `ReceiptResult` — `receipt.py:85`; `Event` — `receipt.py:144`; the store is authoritative — `append_event` — `state.py:968`; the JSONL export — `JSONLEventSink` — `receipt.py:346` | `test_T11_every_demo_receipt_carries_every_field_in_the_spec`, `test_T11_every_demo_receipt_parses_back_into_a_Receipt` |
...
| "narrow it at runtime with `ctrlrun delegate`" | `Control.delegate` — `control.py:1480`; `Authority.plan_delegation` — `authority.py:878`; `ctrlrun delegate` — `cli/main.py:660` | `test_t75_the_delegation_authorizes_an_action_within_its_limits` |
...
| "`ctrlrun revoke` cuts a chain of any depth with one write" | `Control.revoke` — `control.py:1496` — writes one row (`state.py:1067`) and visits no children; every evaluation walks to the root | `test_t78_a_revoked_parent_denies_its_grandchild`, `test_put_delegation_is_never_an_upsert` |
