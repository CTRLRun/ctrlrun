...
| "`ctrlrun init` writes a starter" | `init` — `cli/main.py:342` | CI's `package` job runs `ctrlrun init` from the wheel and asserts `ctrlrun.yaml` exists |
| "The human runs `ctrlrun approve <request id>` and the agent calls again inside `ctrlrun.with_approval(request_id)`" | `approve` — `cli/main.py:366`; `with_approval` — `control.py:164`; `ApprovalRequired` (`errors.py:88`) carries `request_id` | `test_T2_a_mutated_action_presenting_the_approval_raises_ApprovalMismatch` (the granted path first), `test_T4_replaying_the_approval_raises_ApprovalMismatch_with_reason_consumed` |
...
| "Point the MCP client at the gateway instead of at the tool server" | `Gateway.handle` — `gateway/server.py:379`; `serve` — `gateway/__init__.py:41` | `test_T19_the_upstream_receives_the_canonical_arguments` |
...
| "narrow it at runtime with `ctrlrun delegate`" | `Control.delegate` — `control.py:1647`; `Authority.plan_delegation` — `authority.py:877`; `ctrlrun delegate` — `cli/main.py:900` | `test_t75_the_delegation_authorizes_an_action_within_its_limits` |
...
| "`ctrlrun revoke` cuts a chain of any depth with one write" | `Control.revoke` — `control.py:1663` — writes one row — `revoke_delegation` — `state.py:554` and visits no children; every evaluation walks to the root | `test_t78_a_revoked_parent_denies_its_grandchild`, `test_put_delegation_is_never_an_upsert` |
...
| "`ctrlrun stats` gives you the numbers" | `stats` — `cli/main.py:729`; counted from `would_have.blocked_reason` and nothing else | `test_T86_stats_counts_what_observe_mode_recorded`, `test_T86_stats_reaches_no_network` |
...
| "`ctrlrun approve`, `deny`, `resolve`, `inspect`, `receipts` and `stats` work from the shell against any store" | `approve` — `cli/main.py:366`; `receipts` — `cli/main.py:428`; `effects` — `cli/main.py:502`; `resolve` — `cli/main.py:520`; `inspect` — `cli/main.py:554`; `stats` — `cli/main.py:729`; every one takes `--store-url` (SPEC-v0.6 §9.4) | `test_T10_resolve_failed_permits_a_retry`, `test_T18_inspect_json_emits_the_inspection_schema`, `test_T86_stats_counts_what_observe_mode_recorded`, `test_verify_chain_reads_a_postgres_store_through_store_url` |
