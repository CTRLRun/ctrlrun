...
description: "Route approval requests to a webhook with WebhookApprovalProvider: one signed POST per request, a human answers in Slack or any chat tool, the answer comes back signed to the gateway's endpoint, and the same grant call the CLI uses writes it."
---
...
<Steps>
  ...
  <Step title="Receive the request">
    One POST per `APPROVAL_REQUESTED`, with the `CTRLRun-Signature` header, an HMAC-SHA256 over
    `timestamp.body` computed on the exact bytes sent. Verify it before you read the body.
  ...
  <Step title="Send the answer back">
    ...
    ```json
    {"decision": "grant", "approver": "dana@example.com"}
    ```
    ...
    the reason on a bad signature, an unknown request id, an expired request, or a request
    already answered the other way.
  </Step>

  <Step title="Watch it land">
    ```bash
    ctrlrun inspect apr_…     # the request's history, then the grant
    ctrlrun receipts --last 1  # approver: dana@example.com
    ```
