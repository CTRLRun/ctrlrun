...
| "Point the MCP client at the gateway instead of at the tool server" | `Gateway.handle` — `gateway/server.py:436`; `serve` — `gateway/__init__.py:40` | `test_T19_the_upstream_receives_the_canonical_arguments` |
...
| `production/index` | the **Not yet** list: no soak of the length the roadmap asks for, no external security audit, no third-party review of the kernel, no sector packs | three are stated rather than measured, because nothing in a repository can measure an absence; the soak's week is **derived** from the published run, so that line leaves on its own the day a week is finally run rather than outliving the thing it reports. The list lives inside the generated block so it cannot be scrolled past. `test_the_not_yet_list_is_inside_the_block_and_not_below_it` and `test_the_not_yet_list_grows_the_week_until_the_week_is_run` assert both; removing a stated line is its own pull request with the row that makes the new sentence true |
...
| `production/soak` | every number on the page, and that the roadmap's exit criterion is **not met** by a twenty-minute run | rendered by `tools/docs_audit/render_soak.py` from `research/soak/results/*.json`; `test_the_soak_page_is_the_render_of_the_published_results` and `test_the_soak_page_does_not_let_a_reader_believe_the_criterion_was_met` |
