...
| "what an agent can do autonomously, what requires human approval, and what is blocked" | `Decision.ALLOW / APPROVE / DENY` — `policy.py:46`. Exactly three members. | `test_T6_unknown_action_is_denied_with_reason_unknown_action` |
| "binds approvals to the exact action" | `ApprovalRequest.action_hash` — `approval.py:77`; `consume_approval(approval_id, action_hash)` — `approval.py:274` | `test_T2_a_mutated_action_presenting_the_approval_raises_ApprovalMismatch`, `test_T2_any_material_change_invalidates_the_approval` |
| "blocks duplicate execution attempts for the same logical effect" | `reserve_effect` — `state.py:292`, inside `BEGIN IMMEDIATE` (`state.py:742`) against `effect_key TEXT PRIMARY KEY` (`state.py:69`) | `test_T3_exactly_one_agent_reserves_and_seven_are_blocked` (8 OS processes), `test_T3_the_fake_remote_is_called_exactly_once` |
...
| "Autonomy belongs to the action, not the agent." | `Policy.evaluate(action_name, arguments)` — `policy.py:144`. The signature takes **no principal**: the agent's identity is structurally incapable of changing a decision. | `test_T6_an_action_name_is_matched_exactly` |
