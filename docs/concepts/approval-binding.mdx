...
description: "An approval is bound to the hash of the exact action a human saw, is used once, expires, and is consumed in the same atomic write that reserves the effect, so a mutated or replayed action is refused."
---
