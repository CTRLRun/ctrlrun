---
title: "Verify in CI"
description: "Run ctrlrun verify against your policy on every push with the CTRLRun GitHub Action, read the two shapes of report, understand the N/A line, and publish the badge that means the declared guarantees pass."
---
...
- Exit 2, `nothing could be exercised`: the policy has no `approve` rule, no `effect:` template
  and no grants, so `0/0`, which is never a pass.
