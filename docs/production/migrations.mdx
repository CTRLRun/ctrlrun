...
The five are exhaustive and the store classifies into exactly one. Anything it cannot place is
refused, not opened: a sixth shape nobody thought of is a database nobody has reasoned about.
...
- **It does not migrate a database that is not CTRLRun's.** A database with foreign tables and no
  effects table is refused, naming what it found.
