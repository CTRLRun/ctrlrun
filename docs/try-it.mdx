...
Real Python and the released `ctrlrun` wheel from PyPI load into this tab. Below is one
protected refund under one policy, with a fake Stripe that commits before its reply can go
missing. Change the amount, lose the reply, approve one amount and execute another, and read
what CTRLRun did. Every line in the box was produced by `ctrlrun` here: the page owns the
controls and nothing else. Nothing is sent anywhere; the store is in memory and no socket is
opened.

The policy the refund runs under, in cents:
...
5. **`txn_4`**, **€500**, with *lose the reply* ticked — the remote commits, the reply is lost,
   ...
   for: [Outcomes and AMBIGUOUS](/concepts/outcomes-and-ambiguous).
6. Back to **`txn_1`**, **€500** — `DuplicateEffect`. One logical effect,
   [one reservation](/concepts/effect-keys).

Every refusal above is the exception your own code would catch, with the reason it carries.
[Approval binding](/concepts/approval-binding) and [Decisions](/concepts/decisions) are the two
...
- [Protect your first action](/get-started/quickstart): the same thing, in your own process.
