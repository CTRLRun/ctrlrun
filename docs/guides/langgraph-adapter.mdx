...
description: "Route an approve decision through LangGraph's own interrupt() with ctrlrun-langgraph: the operator builds the Control, @protect(wait=True) sends the request out through the interrupt, Command(resume=...) carries the human's answer and its arguments back, and core re-checks them against the hash."
---
