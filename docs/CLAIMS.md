...
| `production/index` | the **Not yet** list: no external security audit, no third-party review of the kernel, no sector packs | stated rather than measured, because nothing in a repository can measure an absence. It lives inside the generated block so it cannot be scrolled past, and `test_the_not_yet_list_is_inside_the_block_and_not_below_it` asserts that; removing a line is its own pull request with the row that makes the new sentence true |
