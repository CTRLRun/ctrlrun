...
description: "You already run an MCP server. Two commands put CTRLRun between the agent and it: policy, approval binding, effect keys and receipts on every tools/call, with the JSON-RPC errors the agent sees on deny and on approval-required."
---
