...
description: "Move the store from SQLite to Postgres when more than one host must share it: pip install ctrlrun[postgres], a connection URL, a schema, the grants the role needs, migrations at open, and what a lost COMMIT means."
---
...
<Steps>
  ...
  <Step title="Create the schema and grant the role">
    ...
    `CREATE` on the schema is what migrations need; a read-only reporting role needs `USAGE`
    and `SELECT` on the tables, and `ctrlrun receipts --verify-chain` runs fine on it. The
...
schema it creates and drops, never against yours. The store conformance suite,
`python -m ctrlrun.conformance.store`, grades your database with the same cases SQLite passes.
