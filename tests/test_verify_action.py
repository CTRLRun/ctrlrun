...
def test_T118_ci_asserts_the_two_shapes_the_specification_names():
    ...
    assert 'test "$AUTHORITY" = "verified 21/21"' in script
    assert 'test "$TEMPLATES" = "verified 11/11"' in script
    assert 'test "$AUTHORITY_NA" = "2"' in script
    assert 'test "$TEMPLATES_NA" = "12"' in script
...
def test_T118_the_two_configurations_really_do_report_those_shapes():
    ...
    assert authority.badge["message"] == "verified 21/21"
    ...
    assert templates.not_applicable == 12
...
def test_T119_the_colour_is_about_failures_and_has_no_amber_for_not_applicable(
    ...
    assert passing.not_applicable == 12
