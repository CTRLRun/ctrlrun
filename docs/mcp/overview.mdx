...
description: "CTRLRun works with MCP in three ways: the gateway enforces policy, approval binding, effect keys and receipts in front of any MCP server; this documentation is an MCP server; and the gateway is listed for discovery. An operator server for approvers is planned, not shipped."
---
