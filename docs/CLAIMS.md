...
| "controlling consequential AI-agent actions" | `@protect` — `control.py:1801`; `context()` — `control.py:132` | `test_T6_unknown_action_raises_ActionDenied_with_reason_unknown_action` |
...
| "blocks duplicate execution attempts for the same logical effect" | `reserve_effect` — `state.py:353`, decided inside the `BEGIN IMMEDIATE` of `_authorize_and_reserve` (`state.py:1328`) against `effect_key TEXT PRIMARY KEY` (`migrations.py:107`; `COLLATE "C"` on Postgres, §4.4) | `test_T3_exactly_one_agent_reserves_and_seven_are_blocked` (8 OS processes), `test_T3_the_fake_remote_is_called_exactly_once` |
...
| "records what actually happened" | `ReceiptResult` — `receipt.py:85`; `Event` — `receipt.py:144`; the store is authoritative — `append_event` — `state.py:969`; the JSONL export — `JSONLEventSink` — `receipt.py:346` | `test_T11_every_demo_receipt_carries_every_field_in_the_spec`, `test_T11_every_demo_receipt_parses_back_into_a_Receipt` |
...
| "narrow it at runtime with `ctrlrun delegate`" | `Control.delegate` — `control.py:1480`; `Authority.plan_delegation` — `authority.py:878`; `ctrlrun delegate` — `cli/main.py:877` | `test_t75_the_delegation_authorizes_an_action_within_its_limits` |
...
| "`ctrlrun revoke` cuts a chain of any depth with one write" | `Control.revoke` — `control.py:1496` — writes one row (`state.py:1068`) and visits no children; every evaluation walks to the root | `test_t78_a_revoked_parent_denies_its_grandchild`, `test_put_delegation_is_never_an_upsert` |
...
| "`ctrlrun stats` gives you the numbers" | `stats` — `cli/main.py:642`; counted from `would_have.blocked_reason` and nothing else | `test_T86_stats_counts_what_observe_mode_recorded`, `test_T86_stats_reaches_no_network` |
...
| Claim | Code | Proof |
|---|---|---|
| "You probably do not need an adapter" | Three ways in, and `@protect` (`control.py:1801`) covers this process while the gateway covers MCP — an adapter buys only the interrupt | `test_T139_the_adapter_section_says_when_you_do_not_need_one_up_front` |
