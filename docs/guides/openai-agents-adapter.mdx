...
description: "Route an approve decision through the OpenAI Agents SDK's tool-approval interruption with ctrlrun-openai-agents: protected_tool wires needs_approval to the policy, gate.run returns the kernel's exceptions as themselves, and the binding is attribution because the SDK records that a call was approved, not its arguments."
---
