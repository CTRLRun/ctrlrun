...
| "blocks duplicate execution attempts for the same logical effect" | `reserve_effect` — `state.py:380`, decided inside the `BEGIN IMMEDIATE` of `_authorize_and_reserve` (`state.py:1346`) against `effect_key TEXT PRIMARY KEY` (`state.py:71`) | `test_T3_exactly_one_agent_reserves_and_seven_are_blocked` (8 OS processes), `test_T3_the_fake_remote_is_called_exactly_once` |
