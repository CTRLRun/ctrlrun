...
def test_T118_ci_asserts_the_two_shapes_the_specification_names():
    ...
    assert 'test "$AUTHORITY" = "verified 26/26"' in script
    assert 'test "$TEMPLATES" = "verified 13/13"' in script
...
def test_T118_the_two_configurations_really_do_report_those_shapes():
    ...
    assert authority.badge["message"] == "verified 26/26"
    ...
    assert templates.badge["message"] == "verified 13/13"
...
def test_T120_a_configuration_with_not_applicable_guarantees_still_writes_a_badge():
    ...
    assert report.badge["message"] == "verified 13/13"
