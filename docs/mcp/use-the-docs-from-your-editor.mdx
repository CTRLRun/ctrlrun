...
description: "This documentation is an MCP server. One configuration block adds it to Cursor, VS Code or any MCP client, and the assistant answers questions about CTRLRun from these pages instead of from memory."
---
...
One image, when the site is live: the editor's MCP settings panel with `ctrlrun-docs` listed
and its one tool, `search`, beside a chat pane where the first question above was asked and
answered with the link to the AMBIGUOUS page visible. 1600×1000, the editor's default theme, no
