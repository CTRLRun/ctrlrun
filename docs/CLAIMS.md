...
| "blocks duplicate execution attempts for the same logical effect" | `reserve_effect` — `state.py:380`, decided inside the `BEGIN IMMEDIATE` of `_authorize_and_reserve` (`state.py:1363`) against `effect_key TEXT PRIMARY KEY` (`state.py:71`) | `test_T3_exactly_one_agent_reserves_and_seven_are_blocked` (8 OS processes), `test_T3_the_fake_remote_is_called_exactly_once` |
...
| "records what actually happened" | `ReceiptResult` — `receipt.py:85`; `Event` — `receipt.py:144`; JSONL `append_event` — `state.py:607` | `test_T11_every_demo_receipt_carries_every_field_in_the_spec`, `test_T11_every_demo_receipt_parses_back_into_a_Receipt` |
