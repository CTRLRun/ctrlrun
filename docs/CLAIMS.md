...
| "blocks duplicate execution attempts for the same logical effect" | `reserve_effect` — `state.py:316`, decided inside the `BEGIN IMMEDIATE` of `_authorize_and_reserve` (`state.py:1285`) against `effect_key TEXT PRIMARY KEY` (`migrations.py:85`) | `test_T3_exactly_one_agent_reserves_and_seven_are_blocked` (8 OS processes), `test_T3_the_fake_remote_is_called_exactly_once` |
...
| "records what actually happened" | `ReceiptResult` — `receipt.py:85`; `Event` — `receipt.py:144`; JSONL `append_event` — `state.py:543` | `test_T11_every_demo_receipt_carries_every_field_in_the_spec`, `test_T11_every_demo_receipt_parses_back_into_a_Receipt` |
...
| "`ctrlrun revoke` cuts a chain of any depth with one write" | `Control.revoke` — `control.py:1484` — writes one row (`state.py:1025`) and visits no children; every evaluation walks to the root | `test_t78_a_revoked_parent_denies_its_grandchild`, `test_put_delegation_is_never_an_upsert` |
