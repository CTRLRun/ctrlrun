...
| "controlling consequential AI-agent actions" | `@protect` — `control.py:1864`; `context()` — `control.py:132` | `test_T6_unknown_action_raises_ActionDenied_with_reason_unknown_action` |
| "what an agent can do autonomously, what requires human approval, and what is blocked" | `Decision.ALLOW / APPROVE / DENY` — `policy.py:202`. Exactly three members. | `test_T6_unknown_action_is_denied_with_reason_unknown_action` |
| "binds approvals to the exact action" | `ApprovalRequest.action_hash` — `approval.py:299`; `consume_approval(approval_id, action_hash)` — `approval.py:299` | `test_T2_a_mutated_action_presenting_the_approval_raises_ApprovalMismatch`, `test_T2_any_material_change_invalidates_the_approval` |
...
| "stops blind retries when an execution outcome is uncertain" | Only `NotExecuted` maps to `FAILED` — `control.py:960`. Every other exception, timeouts included, yields `AMBIGUOUS`. | `test_T1_a_blind_retry_writes_a_blocked_receipt`, `test_T1_the_ambiguous_record_survives_the_blocked_retry` |
| "records what actually happened" | `ReceiptResult` — `receipt.py:409`; `Event` — `receipt.py:409`; the store is authoritative — `append_event` — `state.py:513`; the JSONL export — `JSONLEventSink` — `receipt.py:409` | `test_T11_every_demo_receipt_carries_every_field_in_the_spec`, `test_T11_every_demo_receipt_parses_back_into_a_Receipt` |
| "Autonomy belongs to the action, not the agent." | `Policy.evaluate(action)` — `policy.py:145` — passes only the action's **name and arguments** to `_ActionPolicy.evaluate` (`policy.py:145`), whose signature has no principal in it. A rule cannot read who is acting even by accident. `agent_eq` and `user_eq` are refused at load by `RESERVED_ARGUMENTS` (`policy.py:145`) rather than silently matching nothing. | `test_T6_an_action_name_is_matched_exactly`, `test_a_condition_naming_an_action_field_is_refused_at_load` |
...
| "Declare their effect and resource templates there" | `Policy.effect_template` / `resource_template` — `policy.py:578`; `McpOptions` — `policy.py:578` | `test_T16_a_v2_document_loads_and_exposes_its_templates`, `test_T16_a_decorator_and_a_policy_template_produce_the_same_action_hash` |
...
| Claim | Code | Proof |
|---|---|---|
| "A policy … cannot see who is asking — deliberately, since v0.1" | `Policy.evaluate` still takes only the action's name and arguments; `RESERVED_ARGUMENTS` — `policy.py:145` — refuses `agent_eq` and every other principal-addressing condition at load, in a document of **every** schema version | `test_T74b_a_reserved_name_in_a_policy_rule_is_a_load_error`, `test_T74b_a_reserved_name_in_a_grant_constraint_is_a_load_error` |
| "`authority:` is the second axis" | `Authority.evaluate` — `authority.py:807`; `Control._authority_result` — `control.py:395` | `test_T67_a_principal_with_no_grant_is_denied` |
...
| "authority first" | `Control.execute` evaluates authority before policy and a denial appends `AUTHORITY_DENIED` and never `POLICY_EVALUATED` — `control.py:529` | `test_T74_a_denial_leaves_no_pending_approval_request` |
| "narrow it at runtime with `ctrlrun delegate`" | `Control.delegate` — `control.py:1531`; `Authority.plan_delegation` — `authority.py:878`; `ctrlrun delegate` — `cli/main.py:919` | `test_t75_the_delegation_authorizes_an_action_within_its_limits` |
...
| "`mode: observe` … records what *would* have been blocked, without blocking anything" | `_parse_mode` — `policy.py:708`; `Control._observed` — `control.py:702`; `_WouldHave` — `receipt.py:190`; `ReceiptResult.OBSERVED` — `receipt.py:190` | `test_T82_observe_executes_what_enforce_would_deny`, `test_T83_a_duplicate_is_recorded_and_still_runs` |
| "One top-level line" | `mode:` is refused anywhere but the top level — `reject_nested_mode`, `policy.py:727` | `test_T84_mode_is_refused_anywhere_but_the_top_level` |
