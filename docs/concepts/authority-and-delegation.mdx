...
description: "Authority is the second axis: a grant says which principal may propose which action, opt-in and then fail-closed, evaluated before the policy. Delegation only narrows a grant, at creation and at every evaluation. Identity is consumed from a header or a JWT, never issued."
---
