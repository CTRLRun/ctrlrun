...
description: "Two commands put policy, approval binding, effect keys and receipts in front of an existing MCP server, in any language, with no change to the agent or the server: ctrlrun gateway --upstream, and a policy that names the tools."
---
